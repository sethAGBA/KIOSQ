...
- [x] 2. Compléter le `Numeric_Helper` et le couvrir par des tests de propriétés
  - [x] 2.1 Vérifier et compléter `numericRow` / `numericRows` dans `api/_lib/response.ts`
  ...
  - [x]* 2.2 Écrire le test de propriété pour `numericRow`
...
- [ ] 3. Créer `api/categories/[id].ts` (PATCH + DELETE)
  - [x] 3.1 Implémenter le handler `api/categories/[id].ts`
...
- [x] 4. Créer `api/dashboard/stats/index.ts`
  - [x] 4.1 Créer le dossier et implémenter le handler `api/dashboard/stats/index.ts`
  ...
  - [x]* 4.2 Écrire le test de propriété — `caParMois` complétude sur 12 mois
...
- [ ] 5. Mettre à jour `api/commandes-fournisseurs/[id].ts` — logique de réception de stock
  - [x] 5.1 Ajouter la mise à jour du stock produits au PATCH statut `recu` / `recu_partiel`
  ...
  - [x]* 5.2 Écrire le test de propriété — invariant financier des paiements CF
...
- [x] 6. Checkpoint — s'assurer que les tests existants passent
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Mettre à jour `server.ts` — enregistrer les nouvelles routes
  - [x] 7.1 Ajouter les routes manquantes dans `server.ts`
...
- [x] 8. Ajouter `dashboardApi.stats()` dans `src/lib/api.ts`
  - [x] 8.1 Déclarer l'interface `DashboardStats` et ajouter l'objet `dashboardApi` dans `src/lib/api.ts`
...
- [x] 9. Mettre à jour `DashboardPage.tsx` — remplacer `mockDataCA` par l'API
  - [x] 9.1 Câbler `DashboardPage.tsx` sur `dashboardApi.stats()`
...
- [x] 10. Corriger la gestion d'erreur du chargement initial dans `AppStore`
  - [x] 10.1 Ajouter un champ `error: string | null` dans l'`AppStore` et le propager
...
- [x] 11. Compléter le script de seed avec des commandes, factures et commandes fournisseurs
  - [x] 11.1 Enrichir `db/seed.ts` avec des données historiques multi-mois
...
- [x] 13. Checkpoint final — exécuter tous les tests et valider l'intégration
  - Ensure all tests pass, ask the user if questions arise.
